...
## Corpus Check
- 33 files · ~47,430 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 634 nodes · 1223 edges · 25 communities
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 59 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a107c551`
...
### Community 6 - "tdq_checkportable.py"
Cohesion: 0.11
...
### Community 10 - "check_canvas_layout.py"
Cohesion: 0.16
...
Cohesion: 0.14
Nodes (18): _atomic_write(), lane_label(), plugin_root_cmd(), _pop_json_flag(), prompt_context_last(), prompt_context_path(), prompt_context_save(), Xoá dòng của session này (đầu turn). Dòng session khác giữ nguyên. (+10 more)
...
### Community 16 - "cli"
Cohesion: 0.13
Nodes (20): cli(), _dong_so_request_cu(), _echo_state(), effective_mode(), ghi_moc_phase(), _info(), log_enabled(), now_iso() (+12 more)
...
### Community 18 - "turn_snapshot"
Cohesion: 0.19
Nodes (13): _git(), stdout (bytes) của lệnh git, hoặc None khi không chạy được., Gốc repo (porcelain in path theo gốc, không theo cwd). None nếu không phải repo., Dấu nhận dạng file untracked → (dấu, số byte đã đọc).      Ưu tiên NỘI DUNG: mti, Vân tay trạng thái làm việc của repo, hoặc None khi không lấy được.      Gồm cả, Path đang khác so với HEAD (bỏ cờ trạng thái, rename lấy vế đích).      Cùng vùn, Trạng thái đầu turn: log hôm nay + vân tay repo + danh sách path đang bẩn     +, repo_root() (+5 more)

### Community 19 - "_cli_approve"
Cohesion: 0.27
...
### Community 21 - "tdq-workflow — Plugin Claude Code"
Cohesion: 0.22
...
### Community 23 - "save"
Cohesion: 0.29
Nodes (8): find_shadow_states(), Project root cho state: TDQ_PROJECT_DIR > git root > thư mục đã có state > cwd., State/mirror lạc chỗ: state.json ngoài root, hoặc STATE.md mồ côi (S6)., Ghi nguyên tử state.json + sinh lại STATE.md (S1, mirror).      expect_updated_a, resolve_project_dir(), save(), state_md_path(), state_path()
...
- **Why does `turn_log_append()` connect `_common.py` to `cli`, `tdq_state.py`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `main()` connect `_common.py` to `tdq_state.py`, `cli`, `turn_snapshot`, `_cli_approve`, `effective_lane`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
- **Why does `payload_cwd()` connect `_common.py` to `save`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
