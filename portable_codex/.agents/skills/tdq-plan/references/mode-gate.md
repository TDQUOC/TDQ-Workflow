...
```
python3 scripts/tdq_state.py modes --json
```
...
`scripts/tdq_team.py audit` exits non-zero when the leader invents a group outside that set to
keep work. The set is the constant `LY_DO_GIU` in `scripts/tdq_team.py`; the full lookup table
