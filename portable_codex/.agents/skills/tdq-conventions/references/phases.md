...
Regenerate: `python3 "./scripts/tdq_state.py" phases-doc --plugin-root > <file>`.
...
| `implement` | plan_approved = true and implement_mode is settled | Do the whole plan in one turn, mark [~] when a task starts, red→green, flip to [x] as soon as it passes | `python3 "./scripts/tdq_state.py" set phase=qc` | Every task in the plan is ticked [x] | Stopping midway; batching the ticks at the end of the turn; leaving several tasks marked [~] |
...
diagram: python3 "./scripts/tdq_state.py" approve diagram <path> --by "<the user's sentence verbatim>"
