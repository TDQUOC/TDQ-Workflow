...
5. User approves → run `python3 "./scripts/tdq_state.py" approve quick [--no-qc] --by "<nguyên văn>"` (`--no-qc` ONLY when the user says so explicitly — silence about QC means QC stays ON).
