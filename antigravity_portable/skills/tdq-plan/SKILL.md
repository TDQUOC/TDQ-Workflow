...
1. **Pick the mode to PROPOSE.** Which modes exist depends on the machine: read them with
   ...
   ```
   python3 "~/.gemini/config/plugins/tdq-workflow/scripts/tdq_bench.py" simulate --plan docs/tdq/plan/<slug>.md \
...
6. **The user approves → record it IMMEDIATELY, then ask about the run mode in the SAME turn:**
   ...
   (`MODE_LABELS`/`MODE_ALIASES` in `~/.gemini/config/plugins/tdq-workflow/scripts/tdq_state.py`). The user answers → re-run with
