...
1b. **Draw the diagram — mandatory here too.** Express is a shorter path, not a path with the
   ...
   `python3 "~/.gemini/antigravity-cli/tdq/scripts/tdq_mindmap.py" sinh <feature-slug>`. Fill it in, then
...
```
5. The user approves → run `python3 "~/.gemini/antigravity-cli/tdq/scripts/tdq_state.py" approve quick [--no-qc] --by "<the user's sentence verbatim>"` (`--no-qc` ONLY when the user says so explicitly — silence about QC means QC stays ON).
...
7. Implement end-to-end in 1 turn. **Before typing the first line of code, count the tasks
   ...
   - **3 or more** → hand them to sub-agent `tdq-implementer`, one agent per task, issued in
     ...
     same cap `python3 ~/.gemini/antigravity-cli/tdq/scripts/tdq_team.py cum` applies in the deep pipeline (task 5 prints
...
reading a specific file → grep/read. The graph holds only `~/.gemini/antigravity-cli/tdq/scripts/` and `~/.gemini/antigravity-cli/tdq/hooks/`; tests and
