...
python3 "~/.gemini/config/plugins/tdq-workflow/scripts/tdq_state.py" set loai_request=<loại> nhanh_goc=<cũ> nhanh_request=<mới>
```

## Self-check

- `python3 "~/.gemini/config/plugins/tdq-workflow/scripts/tdq_state.py" get` shows all three keys filled.
