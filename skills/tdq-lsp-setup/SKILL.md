...
## The ladder — `python3 scripts/tdq_lsp.py kiem`
...
Which marker belongs to which language lives in `scripts/tdq_lsp.py`, not here — run the command.
...
`python3 scripts/tdq_lsp.py danh-thuc` wakes the daemon; `nha` releases the embedding model right
...
Done when: `python3 scripts/tdq_lsp.py kiem` prints seven rungs with no actionable gap and exits 0,
