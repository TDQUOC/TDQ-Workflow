...
Layering: Tavily is the FAST search tier. DEEP search (multi-source ranking, deep URL reads) defaults to the 2-phase hybrid flow — `search-scout` (Claude + Tavily) in parallel with `search-runner` (agy) for breadth, then `search-runner` agents for depth; trigger criteria and flow in [deep-search.md](deep-search.md). Tavily also powers the scout slot and serves as the fallback when the agy engine reports `engine-failed` ≥2 times in a row.
