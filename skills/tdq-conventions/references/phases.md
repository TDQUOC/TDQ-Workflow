...
|---|---|---|---|---|---|
| `no_state` | Chưa có request TDQ nào đang mở | Hỏi user chọn lane rồi mở request mới | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-HHMM-slug> <nhanh\|chuyen-sau>` | state.json có active_request và lane | Sửa code khi chưa mở request |
| `analyze` | Đã có request, chế độ chuyên sâu (deep) | Đọc code, research, interview user đến khi hết chỗ mơ hồ | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" set phase=spec` | Không còn câu hỏi nào làm thay đổi kết quả | Viết spec khi chưa hết mơ hồ |
...
| `plan` | spec_approved = true | Viết plan kèm mode ĐỀ XUẤT, đăng ký plan_file, trình rồi DỪNG chờ duyệt | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve plan --by "<nguyên văn>"` | plan_approved = true | Sửa code khi plan chưa duyệt; bắt user nói mode mới chịu ghi nhận duyệt |
| `mode` | plan_approved = true mà implement_mode chưa chốt | Giải thích ngắn gọn 2 mode rồi hỏi user chọn, DỪNG chờ trả lời | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve plan --mode <main\|subagent> --by "<nguyên văn>"` | implement_mode khác null | Sửa code khi chưa chốt mode; tự chọn mode thay user |
...
| `idle` | Đã xong hoặc chưa mở request | Chờ yêu cầu mới từ user | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-HHMM-slug> <nhanh\|chuyen-sau>` | Có request mới được mở | Đè request cũ còn dở mà chưa hỏi user |
...
```
no_state: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-HHMM-slug> <nhanh|chuyen-sau>
...
plan: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve plan --by "<nguyên văn>"
mode: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve plan --mode <main|subagent> --by "<nguyên văn>"
...
idle: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-HHMM-slug> <nhanh|chuyen-sau>
