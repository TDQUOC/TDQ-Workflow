...
| `spec` | Đã phân tích xong | Viết spec, đăng ký spec_file, trình tóm tắt rồi DỪNG chờ user duyệt | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve spec --by "<nguyên văn câu user>"` | spec_approved = true | Viết plan trong cùng turn với spec; tự suy diễn là user đã duyệt |
| `plan` | spec_approved = true | Hỏi mode thực thi, viết plan, đăng ký plan_file, trình rồi DỪNG chờ duyệt | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve plan --mode <main\|subagent\|external> --by "<nguyên văn>"` | plan_approved = true và implement_mode khác null | Sửa code khi plan chưa duyệt; tự chọn mode thay user |
...
| `quick` | lane = quick | Trình mini-plan ≤10 dòng → chờ duyệt → ghi working log TRƯỚC → rồi mới implement | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve quick [--mode external] --by "<nguyên văn câu user>"` | quick_approved = true, log đã ghi, việc đã validate, phase đã về idle | Implement trước khi ghi working log |
...
plan: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" approve plan --mode <main|subagent|external> --by "<nguyên văn>"
...
6. Đóng việc: chạy `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" set phase=idle` — terminal của lane quick
