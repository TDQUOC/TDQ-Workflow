...
|---|---|---|---|---|---|
| `no_state` | Chưa có request TDQ nào đang mở | Hỏi user chọn lane rồi mở request mới | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-slug> <nhanh\|chuyen-sau>` | state.json có active_request và lane | Sửa code khi chưa mở request |
...
| `idle` | Đã xong hoặc chưa mở request | Chờ yêu cầu mới từ user | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-slug> <nhanh\|chuyen-sau>` | Có request mới được mở | Đè request cũ còn dở mà chưa hỏi user |
...
```
no_state: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-slug> <nhanh|chuyen-sau>
...
idle: python3 "${CLAUDE_PLUGIN_ROOT}/scripts/tdq_state.py" init <YYYY-MM-DD-slug> <nhanh|chuyen-sau>
