...
1b. **Draw the diagram — mandatory here too.** Express is a shorter path, not a path with the
   ...
   `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_mindmap.py" sinh <feature-slug>`. Fill it in, then
...
```
5. The user approves → run `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" approve quick [--no-qc] --by "<the user's sentence verbatim>"` (`--no-qc` ONLY when the user says so explicitly — silence about QC means QC stays ON).
