...
2. **Tự review.** Đọc lại tìm chỗ hổng/mâu thuẫn, sửa. Chạy máy kiểm
   (R8 kiểm §3b): `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/doc_lint.py" docs/tdq/spec/<slug>.md`
   đến khi exit 0.
...
5. **User duyệt → ghi nhận NGAY:**
   ```
   python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" approve spec --by "<nguyên văn câu user>"
   ```
...
Bước kế tiếp: `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" set phase=plan`
