...
|---|---|---|
| Rules | `skills/**`, `agents/*.md`, comments and docstrings in `hooks/` and `scripts/` | **English**, always |
...
4. A turn that changed the repo MUST end with the closing command
   `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_finish.py" --files <files just edited> --log "<summary>" --phase <new phase>`
...
- Read and write state **only** through the CLI:
  `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" <next|get|set|approve|init|reset>`.
