...
3. **Tối ưu.** Cắt phần thừa, kiểm thứ tự phụ thuộc. Đối chiếu 2 luật ánh xạ: mỗi đầu ra
   ...
   `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/doc_lint.py" --pair <spec> <plan>` phải exit 0.
...
`python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" set phase=implement`,
