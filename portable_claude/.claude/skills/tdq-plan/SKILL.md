...
6. **The user approves → record it IMMEDIATELY, then ask about the run mode in the SAME turn:**
   ```
   python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" approve plan --by "<nguyên văn>"
   ```
...
`python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" set phase=implement`, then on to
