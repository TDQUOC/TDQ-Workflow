...
- **The end of the turn is gated, not merely asked for.** While the phase is `implement` and
  ...
  yourself → run `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" pause --ly-do "<why>"`,
...
3. All tasks done: run the full suite EXACTLY ONCE, then close the turn's books with ONE
   command
   `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_finish.py" --files <edited .md files> --log "<tasks done, files changed, test result>" --phase qc`
...
Next step: phase `report` — `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" set phase=report`,
then Part C below.
...
Next step: phase `idle` — `python3 "${CLAUDE_PROJECT_DIR}/.claude/tdq/scripts/tdq_state.py" set phase=idle`
