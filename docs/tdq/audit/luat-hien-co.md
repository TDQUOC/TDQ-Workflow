...
| L221 | `skills/tdq-intake/SKILL.md:99` | rồi sang [tdq-spec](../tdq-spec/SKILL.md) — cùng turn nếu interview đã xong, còn phải | then on to [tdq-spec](../tdq-spec/SKILL.md) — same turn if the interview is finished; if |
| L222 | `skills/tdq-intake/SKILL.md:104` | Chế độ nhanh = rút gọn, KHÔNG cắt bước tư duy. Chín bước thi hành — từ phân tích tới hỏi | Express is a shortened path, NOT a path with thinking steps cut out. The nine |
| L223 | `skills/tdq-intake/SKILL.md:107` | `## Chín bước thi hành`. **BẮT BUỘC mở file đó và đọc hết chín bước trước khi làm bước 1; | **You MUST open that file and read all nine steps before doing step 1; working from memory |
| L224 | `skills/tdq-intake/SKILL.md:108` | cấm làm theo trí nhớ.** Cùng file đó có luôn khuôn mini-plan, luật tick, luật QC và vòng fix. | is banned.** That same file also holds the mini-plan khuôn, the tick rule, the QC rule and |
...
| L277 | `skills/tdq-plan/SKILL.md:58` | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/doc_lint.py" --pair <spec> <plan>` phải exit 0. |  |
...
| L319 | `skills/tdq-spec/SKILL.md:84` | rồi sang [tdq-plan](../tdq-plan/SKILL.md) **NGAY trong cùng turn** — không bắt user | then on to [tdq-plan](../tdq-plan/SKILL.md) **NGAY trong cùng turn** — the user is not |
