...
| L010 | `skills/tdq-build/SKILL.md:77` | Mode là thứ USER đã nói lúc duyệt. Thiếu mode, hoặc bạn nghĩ mode khác hợp hơn → **DỪNG và HỎI**. | The mode is what the USER said at approval. Missing mode, or you think another mode fits |
| L011 | `skills/tdq-build/SKILL.md:83` | `- [~]` chỉ dành cho task LEADER tự làm và vẫn chỉ được đúng một. | allowed); `- [~]` is only for a task the LEADER does personally, and still only one. |
| L012 | `skills/tdq-build/SKILL.md:96` | để dành, chạy đúng 1 lần ở QC. Dán kết quả thật, cấm tuyên bố xong khi chưa chạy. | saved for exactly one run at QC. Paste the real output; never declare done unrun. |
| L013 | `skills/tdq-build/SKILL.md:97` | Đổi `- [~]` thành `- [x]` cho task đó trong plan NGAY — mode `subagent` thì main | 6. Turn `- [~]` into `- [x]` for that task in the plan IMMEDIATELY — in mode `subagent` the |
| L014 | `skills/tdq-build/SKILL.md:113` | [references/qc.md](references/qc.md) mục `## Ba bước thi hành`. **BẮT BUỘC mở file đó và | [references/qc.md](references/qc.md) under `## Ba bước thi hành`. **BẮT BUỘC mở file đó và |
| L015 | `skills/tdq-build/SKILL.md:114` | đọc hết ba bước trước khi chạy hạng mục đầu tiên; cấm làm theo trí nhớ.** Cùng file đó có | file and read all three steps before running the first item; working from memory is banned.** |
| L016 | `skills/tdq-build/SKILL.md:124` | BẮT BUỘC mở file đó và đọc hết bốn bước trước khi viết report; cấm làm theo trí nhớ.** | The execution steps — from writing the report through asking about the commit to merging the request branch back — live in |
...
| L097 | `skills/tdq-build/references/team-mode.md:100` | này nhanh hơn `main`, không phải vì agent con chạy nhanh hơn bạn. | mode beats `main`, not because sub-agents type faster than you. |
| L098 | `skills/tdq-build/references/team-mode.md:113` | VÙNG FILE: scripts/alpha.py, tests/test_alpha.py — CẤM sửa file ngoài danh sách này |  |
| L099 | `skills/tdq-build/references/team-mode.md:114` | TEST: <lệnh kiểm của task> — phải đỏ trước, xanh sau |  |
| L100 | `skills/tdq-build/references/team-mode.md:122` | Kèm đường dẫn spec và plan trong phần thân prompt. Agent con KHÔNG đọc được hội thoại | Include the spec and plan paths in the prompt body. A sub-agent CANNOT read this conversation — |
| L101 | `skills/tdq-build/references/team-mode.md:123` | này — thiếu trường nào là nó phải đoán, và đoán sai thì bạn trả giá lúc merge. | a missing field means it has to guess, and a wrong guess is paid for at merge time. |
| L102 | `skills/tdq-build/references/team-mode.md:186` | Trước khi kết thúc phase implement, tất cả phải đúng: | Before ending phase implement, all of these must hold: |
| L103 | `skills/tdq-build/references/team-mode.md:198` | phải có mặt trong report. Tỉ lệ giao thấp mà không có lý do trong bảng tra nghĩa là bạn | total?** That number must appear in the report. A low delegation ratio with no reason from the |
| L104 | `skills/tdq-build/references/team-mode.md:199` | đã lách luật của user — user chọn mode đội là để có một đội, không phải một lời hứa. | lookup table means you worked around the user's rule — the user picked team mode to get a team, |
...
| L277 | `skills/tdq-plan/SKILL.md:59` | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/doc_lint.py" --pair <spec> <plan>` phải exit 0. |  |
| L278 | `skills/tdq-plan/SKILL.md:65` | Không** set `implement_mode` ở đây — trường đó chỉ được ghi khi ghi nhận duyệt. | Do **NOT** set `implement_mode` here — that field is written only when approval is recorded. |
| L279 | `skills/tdq-plan/SKILL.md:67` | Trình bày & DỪNG.** Viết khối trình plan theo | **Present it, then STOP.** Write the plan block per |
| L280 | `skills/tdq-plan/SKILL.md:86` | Phần nội dung ≤ 10 dòng, là tóm tắt THẬT — cấm thay bằng thông báo suông kiểu "đã ghi | The body is ≤ 10 lines and a REAL summary — swapping it for a bare status line such as "đã ghi |
| L281 | `skills/tdq-plan/SKILL.md:87` | trước đoạn trích: "(khuôn mẫu — áp dụng cho các lần hỏi sau, không phải câu hỏi của | Quoting a template as an example → label it right before the excerpt, in the user's language |
| L282 | `skills/tdq-plan/SKILL.md:91` | User duyệt → ghi nhận NGAY, rồi hỏi cách chạy trong CÙNG turn:** | **The user approves → record it IMMEDIATELY, then ask about the run mode in the SAME turn:** |
| L283 | `skills/tdq-plan/SKILL.md:96` | Cấm hỏi lại thứ user vừa nói. | **skip** the gate below and build right away — re-asking is banned. |
| L284 | `skills/tdq-plan/SKILL.md:97` | Chưa nói mode → state dừng ở phase `mode`, in khối hỏi rồi DỪNG. Khuôn nguyên văn — | No mode named → state stops at phase `mode`; print the question block then STOP. |
| L285 | `skills/tdq-plan/SKILL.md:100` | Ngay dưới hai option phải có đoạn **"Vì sao đề xuất"** dài 1–3 dòng. Cấm nói chung | Right under the two options there MUST be a **"Vì sao đề xuất"** paragraph, 1–3 lines long. |
| L286 | `skills/tdq-plan/SKILL.md:105` | User trả lời → chạy lại lệnh trên kèm `--mode <main\|subagent>` rồi build LUÔN cùng | The user answers → re-run the command with `--mode <main\|subagent>` and build RIGHT AWAY, same turn. |
| L287 | `skills/tdq-plan/SKILL.md:106` | turn. Mode chốt là mode user NÓI (khác đề xuất cũng được); cấm tự chọn thay user. | The settled mode is the USER's, even when it differs from your proposal. |
