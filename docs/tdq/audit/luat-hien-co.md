...
| L176 | `skills/tdq-conventions/references/phases.md:13` | \| `plan` \| spec_approved = true \| Viết plan kèm mode ĐỀ XUẤT, đăng ký plan_file, trình rồi DỪNG chờ duyệt \| `python3 "${CLAUDE_PLUGIN_ROOT}/scr… | \| `plan` \| spec_approved = true and every diagram in the list is approved \| Write the plan with a PROPOSED mode… |
...
| L222 | `skills/tdq-intake/SKILL.md:110` | Chế độ nhanh = rút gọn, KHÔNG cắt bước tư duy. Chín bước thi hành — từ phân tích tới hỏi | Express is a shortened path, NOT a path with thinking steps cut out. The nine |
| L223 | `skills/tdq-intake/SKILL.md:113` | `## Chín bước thi hành`. **BẮT BUỘC mở file đó và đọc hết chín bước trước khi làm bước 1; | **You MUST open that file and read all nine steps before doing step 1; working from memory |
| L224 | `skills/tdq-intake/SKILL.md:114` | cấm làm theo trí nhớ.** Cùng file đó có luôn khuôn mini-plan, luật tick, luật QC và vòng fix. | is banned.** That same file also holds the mini-plan khuôn, the tick rule, the QC rule and |
...
| L275 | `skills/tdq-plan/SKILL.md:10` | Yêu cầu `spec_approved = true`. User duyệt spec xong là viết plan NGAY trong cùng turn. | Requires `spec_approved = true` **and every diagram of the request approved** — `set phase=plan` is refused… |
...
| L319 | `skills/tdq-spec/SKILL.md:84` | rồi sang [tdq-plan](../tdq-plan/SKILL.md) **NGAY trong cùng turn** — không bắt user | then on to [tdq-diagram](../tdq-diagram/SKILL.md) **in that very same turn** — the user is not made to send… |
