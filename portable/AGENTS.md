...
```
Slug: `YYYY-MM-DD-HHMM-<kebab ≤5 từ, không dấu>` (giờ địa phương lúc mở request). Slug cũ
chỉ có ngày vẫn đọc được; mở request mới thì bắt buộc có giờ phút. Một request dùng chung
một slug ở mọi thư mục. `brief/` gộp yêu cầu + kiến thức + hỏi đáp vào một file, đúng 3 mục:
