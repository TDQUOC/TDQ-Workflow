...
| `report` | QC đã PASS | Viết report ≤10 dòng rồi hỏi user có commit không | `python3 scripts/tdq_state.py set phase=idle` | Report đã ghi và user đã được hỏi về commit | Tự commit hoặc push khi user chưa yêu cầu |
